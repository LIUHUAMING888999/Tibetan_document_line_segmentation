...
# 初始化
img_black = np.zeros((2000, 1024, 3), np.uint8)
        ...
        if hang == C:
            ...
            for x_y in range(1,80):
                ...
                for X1_X2_X3 in range(2, 40):  # 中间位置，每块区域基线与前后两条对照差距不能过大
                    if (abs(Collect1_x[X1_X2_X3 + (X1_X2_X3 - 2)] - Collect1_x[X1_X2_X3 + (X1_X2_X3 - 4)]) > 3 and abs(
                            Collect1_x[X1_X2_X3 + (X1_X2_X3 - 1)] - Collect1_x[X1_X2_X3 + (X1_X2_X3 - 3)]) > 3 and abs(
                ...
                cv2.line(img_c, (Collect1_y[(x_y-1)], Collect1_x[(x_y-1)]), (Collect1_y[(x_y)], Collect1_x[(x_y)]), (0, 0, 255), 1)#画线
                Collect_qvyv.append([Collect1_y[(x_y-1)],Collect1_x[(x_y-1)]])
                Collect_qvyv.append([Collect1_y[(x_y-1)],Collect1_x[(x_y-1)]-30])
                Collect_qvyv.append([Collect1_y[(x_y)], Collect1_x[(x_y)] - 30])
...
# img_crop = cv2.bitwise_xor(img_crop,img_yuantu)
# img_crop=cv2.bitwise_not(img_crop)
cv_show('opi', img_c)
cv_show('nan',img_crop)
