...
# 上下对应基线不能差距过大
# 与0357和0358相比，在进行基线前后对照时，先选择行尾基线向前对照，在用第11段前后对照
import math
...
while a_diedai < 2:
    if a_diedai == 1:
        root_path = "D:/dcc/remove_border1/"
    else:
        root_path = 'D:/dcc/' + "03512png12" + "/" + "png%d" % (a_diedai +a_diedai- 3) + "/"
    dir = root_path
    count = 0
    for root, dir, files in os.walk(dir):
        for file in files:
            ...
            a = int(height - (0.225) * height)  # 切到八分之七行
            b = int(height - (0.775) * height)  # 切到八分之一行
                    ...
                    if hang == C:
                        ...
                        for h in range(40):
                            ...
                            if C_a >= 3:  # 统计波峰穿过连通域个数，超过3个就极大削弱约束1
                                if g < b or g > a:
                                    img_crop1 = img_c[hang - 40:hang + 15, g:f]  # 切割波峰附近区域
                                else:
            ...
            # cv_show('opi', img_c)
            # cv2.imwrite('F:/ccrenew/' + "03511png11" + "/" + "png%d" % (a_diedai*2-1) + "/" + str(file), bw2)
            cv2.imwrite('D:/dcc/' + "03514png14" + "/" + str(file), img_c)
            count += 1
            # if count % 400 == 0:
            print(count)
    a_diedai += 1
