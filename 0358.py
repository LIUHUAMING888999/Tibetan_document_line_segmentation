...
while a_diedai < 5:
    if a_diedai == 1:
        ...
        root_path = 'F:/ccrenew/' + "0358png8" + "/" + "png%d" % (a_diedai +a_diedai- 3) + "/"
    dir = root_path
    count = 0
    for root, dir, files in os.walk(dir):
        for file in files:
            ...
            # cv_show('opi', img_c)
            cv2.imwrite('F:/ccrenew/' + "0358png8" + "/" + "png%d" % (a_diedai*2-1) + "/" + str(file), bw2)
            cv2.imwrite('F:/ccrenew/' + "0358png8" + "/" + "png%d" % (a_diedai*2) + "/" + str(file), img_c)
            count += 1
            # if count % 400 == 0:
            print(count)
    a_diedai += 1
