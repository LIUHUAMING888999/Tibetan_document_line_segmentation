...
bw2 = getCorrect(args["image"])  # 切片专用
#
# print(bw2.shape)
# cv_show('bw', bw2)
        ...
        if hang == C:
            ...
            for h in range(40):
                ...
                if C_a >= 3:#统计波峰穿过连通域个数，超过3个就极大削弱约束1
                    ...
                    # aaa=int(50*math.tan(-find_angle))
                    # XXX2=int(XXX2+aaa)
                    # print(int(50 * math.tan(-find_angle)))
                    #
            ...
            for x_y in range(1, 80):
                Collect_qvyv = []  # 上元音区域
                for X1_X2_X3 in range(2, 40):  # 中间位置，每块区域基线与前后两条对照差距不能过大
                    ...
                    if abs(Collect1_x[-3] - Collect1_x[-1]) > 3 and abs(
                        ...
                        Collect1_x[-1] = Collect1_x[-3]
                        Collect1_x[-2] = Collect1_x[-4]
                cv2.line(img_c, (Collect1_y[(x_y - 1)], Collect1_x[(x_y - 1)]),
                         (Collect1_y[(x_y)], Collect1_x[(x_y)]), (0, 0, 255), 1)
                Collect_qvyv.append([Collect1_y[(x_y - 1)], Collect1_x[(x_y - 1)]])
                Collect_qvyv.append([Collect1_y[(x_y - 1)]-1, Collect1_x[(x_y - 1)] - 30])
                Collect_qvyv.append([Collect1_y[(x_y)]-1, Collect1_x[(x_y)] - 30])
                Collect_qvyv.append([Collect1_y[(x_y)], Collect1_x[(x_y)]])
                Collect_qvyv = np.array(Collect_qvyv)
                # print(Collect_qvyv)
                cv2.fillPoly(img_crop, [Collect_qvyv], (255, 255, 255))
# cv_show('name',img_crop)
img_crop=cv2.bitwise_not(img_crop)
img_crop1 = cv2.bitwise_xor(img_crop,img_yuantu)#找到上元音部分
# img_crop2=cv2.bitwise_not(img_crop1)
# cv_show('2',img_crop2)
fimd_image=findcounts(img_crop1)#上元音区域中返回去除上元因的区域拼接
cv_show('name',fimd_image)
img_find=cv2.bitwise_xor(img_crop,fimd_image)#拼接后的图片
# img_find=cv2.bitwise_not(img_find)
# cv_show('kk',img_crop)
# cv2.imwrite('001.18.0.png', img_crop, [cv2.IMWRITE_PNG_COMPRESSION, 0])
cv_show('opi', img_find)

# cv2.imwrite('F:/ccrenew/' + "png2" + "/" + str(file), img_c)
# count += 1
# # if count % 400 == 0:
# print(count)
