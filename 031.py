    ...
    for hang in range(width):
        if hang == C:
            ...
            for h in range(30):
                ...
                e = int((c - d) / 30)  # 分成三十份后，一份的宽
                f = d + e * (h + 1)
                g = d + e * h
                # cv2.line(img_c,(g,0),(g,1024),(0,0,255),2)
                ...
                if find_angle>=0 :
                    ...
                    XXX1 = Collect_xy_start[0]
                    # XXX1 = int(XXX1 + int(3 * math.tan(find_angle)))
                    YYY1 = Collect_xy_start[1]
                    # YYY1=round(YYY1-math.tan(find_angle))
                    Collect_xy_end = onepoint(XX, f, find_hang, g_f, find_angle)
                    # print(Collect_xy_end)
                    XXX2 = Collect_xy_end[0]
                    # aaa = int(3 * math.tan(find_angle))
                    # XXX2 = int(XXX2 + aaa)
                    # print(int(50* math.tan(find_angle)))
                    #
                    YYY2 = Collect_xy_end[1]
            ...
            for x_y in range(1,30):
                Collect1_x_x0=Collect1_x[x_y+(x_y-1)]
                Collect1_x_x1=Collect1_x[x_y+(x_y-1)+1]
                Collect1_y_y0=Collect1_y[x_y+(x_y-1)]
                Collect1_y_y1=Collect1_y[x_y+(x_y-1)+1]
                cv2.line(img_c,(Collect1_y_y0,Collect1_x_x0),(Collect1_y_y1,Collect1_x_x1),(0,0,255),1)
            ...
            #     y=q+r*(ii+1)
            #     u=q+r*ii
            #     img_crop1 = img_c[hang - 30:hang + 7, u:y]  # 切割波峰附近更高区域
            #     for angle1 in np.arange(-5, 5.5, 0.5):  # 从（-5，5）步长为0.5
            #         imgRotation = rotate_bound_white_bg(img_crop1, angle1)  # 旋转
            #         # print(imgRotation.shape)
            #         T_m = Tong_ji(imgRotation)  # 统计区域内波峰波谷，返回其所在行
            ...
            #     # print(g_f)
            #     # print(find_angle)
            #     Collect_xy_start = onepoint(XX, u, find_hang, g_f, find_angle)
            #     # print(Collect_xy_start)
            #     find_angle = find_angle * math.pi / 180
            #
            #     if find_angle >= 0:
            #         XXX1 = Collect_xy_start[0]
            #         YYY1 = Collect_xy_start[1]
            #         Collect_xy_end = onepoint(XX, y, find_hang, g_f, find_angle)
            #         XXX2 = Collect_xy_end[0]
            #
            #         YYY2 = Collect_xy_end[1]
            #
