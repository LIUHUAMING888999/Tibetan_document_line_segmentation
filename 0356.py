...
#上下对应基线不能差距过大
#与0355不同为前后矫正限制大小以及上下基线对应后，调整上下哪条基线
import math
...
# if count<180:
for x_up_down in range(len(peaks_b)-1):#上下基线对照
    for C_x_y in range(79):
        if Collect_up_down_x[x_up_down+1][C_x_y]-Collect_up_down_x[x_up_down][C_x_y]<70:
            ...
            Collect_up_down_x[x_up_down+1][C_x_y]=Collect_up_down_x[x_up_down+1][C_x_y]-20
...
cv_show('opi', img_c)
        ...
        # cv2.imwrite('F:/ccrenew/' + "0355png6" + "/" + str(file), img_c)
        # count += 1
        # # if count % 400 == 0:
        # print(count)
